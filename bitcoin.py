...
def computeDelta(wt, X, Xi):
    ...
    for i in range(len(Xi)):
        ...
        ### code for sim measuere
        mean_cur = np.mean(Xi.iloc[i][:-1])
        num = 0
        #for j in range(length):
        #   num += (src.iloc[j]-mean_src)*(Xi.iloc[i][j]-mean_cur)
        num = np.dot((src-mean_src),(Xi.iloc[i][:-1]-mean_cur))
        den = length * np.std(src) * np.std(Xi.iloc[i][:-1])
        similarity = num/den
	###

        numeratorSum += Xi.iloc[i][-1] * np.exp(wt*similarity)
        denominatorSum += np.exp(wt*similarity)
    
    avg_price_change = numeratorSum/denominatorSum
